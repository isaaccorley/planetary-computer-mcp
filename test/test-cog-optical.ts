#!/usr/bin/env bun
/**
 * Test optical collections with multiple AOIs
 * Collections: sentinel-2-l2a, landsat-c2-l2, hls2-l30, hls2-s30, sentinel-1-rtc
 */
...
// Optical collection test configurations
const COLLECTION_TESTS = [
  // Optical - test various sizes and locations
  {
    collection: "sentinel-2-l2a",
    datetime: "2024-06-01/2024-09-01",
    aois: ["tiny_sf", "small_seattle", "medium_la", "coastal_miami", "mountain_alps"],
    max_cloud: 30,
  },
  {
    collection: "landsat-c2-l2",
    datetime: "2024-06-01/2024-09-01",
    aois: ["small_seattle", "medium_chicago", "large_rockies", "high_lat_alaska"],
    max_cloud: 30,
  },

  // SAR - various terrains
  {
    collection: "sentinel-1-rtc",
    datetime: "2024-01-01/2024-06-30",
    aois: ["small_seattle", "medium_la", "coastal_miami"],
    max_cloud: 100,
  },
...
// Expected dimensions for each (collection, aoi) pair - used for regression detection
const EXPECTED_DIMENSIONS: Record<string, { width: number; height: number }> = {
  "sentinel-2-l2a_tiny-sf": { width: 10, height: 13 },
  "sentinel-2-l2a_small-seattle": { width: 77, height: 113 },
  "sentinel-2-l2a_medium-la": { width: 937, height: 1121 },
  "sentinel-2-l2a_coastal-miami": { width: 1011, height: 1115 },
  "sentinel-2-l2a_alps": { width: 1588, height: 1604 },
  "landsat-c2-l2_small-seattle": { width: 26, height: 39 },
  "landsat-c2-l2_medium-chicago": { width: 281, height: 373 },
  "landsat-c2-l2_large-rockies": { width: 2867, height: 1698 },
  "landsat-c2-l2_alaska": { width: 395, height: 760 },
  "sentinel-1-rtc_small-seattle": { width: 77, height: 113 },
  "sentinel-1-rtc_medium-la": { width: 937, height: 1121 },
  "sentinel-1-rtc_coastal-miami": { width: 1011, height: 1115 },
  "hls2-l30_small-seattle": { width: 26, height: 39 },
  "hls2-l30_medium-la": { width: 313, height: 375 },
  "hls2-s30_small-seattle": { width: 26, height: 39 },
  "hls2-s30_medium-la": { width: 313, height: 375 },
};
...
async function assertImageHasSignal(
  imagePath: string,
  expectedDims?: { width: number; height: number }
): Promise<void> {
  const img = sharp(imagePath);
  const [stats, metadata] = await Promise.all([img.stats(), img.metadata()]);
  const hasSignal = stats.channels.some((channel) => channel.max > 0);
  if (!hasSignal) {
    throw new Error(`Image at ${imagePath} appears empty (all zeros)`);
  }
  if (expectedDims) {
    if (metadata.width !== expectedDims.width || metadata.height !== expectedDims.height) {
      throw new Error(
        `Image dimensions mismatch: expected ${expectedDims.width}x${expectedDims.height}, ` +
...
async function main() {
  console.log("=== Optical Collection Testing ===\n");
  console.log(
    "Testing optical collections: sentinel-2-l2a, landsat-c2-l2, hls2-l30, hls2-s30, sentinel-1-rtc\n"
  );
      ...
      if (result.success) {
        while (success) {
          ...
          try {
            ...
            const dimKey = `${config.collection}_${activeAoi.name}`;
            const expectedDims = EXPECTED_DIMENSIONS[dimKey];
            await assertImageHasSignal(imagePath, expectedDims);
            break;
          } catch (verifyError) {
